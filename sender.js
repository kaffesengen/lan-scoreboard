...
// Lag en avatar-velger i HTML-en dynamisk hvis den ikke finnes
let avatarSelect = document.getElementById("avatarSelect");
if (!avatarSelect) {
  avatarSelect = document.createElement("select");
  avatarSelect.id = "avatarSelect";
  ...
  // Setter inn velgeren før "Legg til"-knappen
  addBtn.parentNode.insertBefore(avatarSelect, addBtn);
}
...
function sendFinishToReceiver() {
  ...
  session.sendMessage(NAMESPACE, payload).catch(() => {
    alert("Kunne ikke sende vinnerne til TV.");
  });
}
...
function addPlayer(name, avatar) {
  ...
  // Lagrer nå avatar sammen med navn og poeng
  players.push({ 
    name: clean, 
    points: 0, 
    avatar: avatar || "🎮" 
...
addBtn.onclick = () => {
  ...
  nameInput.value = "";
  // Velg en tilfeldig emoji til neste mann for moro skyld
  avatarSelect.selectedIndex = Math.floor(Math.random() * AVATARS.length);
  nameInput.focus();
};
...
setInterval(protectCastButtonVisibility, 1000);
