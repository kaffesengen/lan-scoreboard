...
function showWinners(winners) {
  ...
  winnerCardsEl.innerHTML = `
    <div class="win-card dance">
      ...
      <div class="win-avatar">${second ? esc(second.avatar) : "—"}</div> <div class="win-name">${second ? esc(second.name) : "—"}</div>
      <div class="win-pts">${second ? `${second.points} pts` : ""}</div>
    </div>

    <div class="win-card first">
      <div class="win-place">1. plass</div>
      <div class="win-avatar">${first ? esc(first.avatar) : "—"}</div> <div class="win-name">${first ? esc(first.name) : "—"}</div>
      <div class="win-pts">${first ? `${first.points} pts` : ""}</div>
    </div>

    <div class="win-card dance">
      <div class="win-place">3. plass</div>
      <div class="win-avatar">${third ? esc(third.avatar) : "—"}</div> <div class="win-name">${third ? esc(third.name) : "—"}</div>
...
function renderGrid(players) {
  ...
  gridEl.innerHTML = top9.map((p, i) => `
    <div class="player-tile" data-key="${esc(p.name)}">
      ...
      <div class="tile-avatar">${esc(p.avatar)}</div> <div class="tile-name">${esc(p.name)}</div>
